...
#initialize leap things
thresholdLine = 90
finger_heights = [0,0,0,0,0]
...
pianoLine = pygame.draw.line(background, RED, (10, thresholdLine), (190, thresholdLine))
# rthumb = pygame.draw.circle(background, RED, (70, finger_heights[0]), 2)
# rindex = pygame.draw.circle(background, RED, (85, finger_heights[1]), 2)
# rmiddle = pygame.draw.circle(background, RED, (100, finger_heights[2]), 2)
# rring = pygame.draw.circle(background, RED, (115, finger_heights[3]), 2)
# rpinky = pygame.draw.circle(background, RED, (130, finger_heights[4]), 2)

#coordinates of each of the dots on the screen, in order from thumb to pinky
fingerDots = [[70, finger_heights[0]], [85, finger_heights[1]], [100, finger_heights[2]], [115, finger_heights[3]], [130, finger_heights[4]]]
...
def leapControl(frame):
    ...
    for hand in frame.hands:
        ...
        for finger in hand.fingers:
            bone = finger.bone(3)
            finger_heights[finger.type()] = int(bone.next_joint[1])
            # draw a blue circle for the finger
            # print (fingerDots[finger.type()][0],finger_heights[finger.type()])
            pygame.draw.circle(background, BLUE, (fingerDots[finger.type()][0],finger_heights[finger.type()]), 2)
            # screen.blit(background, (0,0)) #erase screen (return to basic background)
            # pygame.display.flip() #redraw with new updates
            if bone.next_joint[1] < thresholdLine:
                    ...
                    if play[finger.type()] == True:
                            ...
                            # pygame.draw.circle(background, RED, (fingerDots[finger.type()][0],finger_heights[finger.type()]), 2)
                            # screen.blit(background, (0,0)) #erase screen (return to basic background)
                            # pygame.display.flip() #redraw with new updates
